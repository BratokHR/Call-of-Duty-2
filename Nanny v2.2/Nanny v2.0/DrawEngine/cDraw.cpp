...
#pragma region Функции движка
int cdraw::trap_R_RegisterFont(const char * fontName, int pointSize) { return ( ( int (*)( const char * fontName, int pointSize ) )d_RegisterFont )( fontName, pointSize ); }
int cdraw::trap_R_RegisterShader( const char * name, int i1, int i2 ) { return ( ( int (*)( const char * name, int, int ) )d_RegisterShader )( name, i1, i2 ); }
void cdraw::trap_R_DrawStretchPic(float x, float y, float width, float height, float s1, float t1, float s2, float t2, float * colour, LPSTR shader) { ( ( void (*)( float x, float y, float width, float height, float s1, float t1, float s2, float t2, float * colour, int shader ) )d_DrawStretchPic )( x, y, width, height, s1, t1, s2, t2, colour, trap_R_RegisterShader((char*)shader, 3, 7) ); }
void cdraw::trap_R_DrawRotatedPic(float x, float y, float width, float height, float s1, float t1, float s2, float t2, float angle, float * colour, LPSTR shader) { ( ( void (*)(float x, float y, float width, float height, float s1, float t1, float s2, float t2, float angle, float * colour, int shader) )d_DrawRotatedPic )(x, y, width, height, s1, t1, s2, t2, angle, colour, trap_R_RegisterShader((char*)shader, 3, 7)); }
void cdraw::trap_R_DrawStringExt(const char * text, int type, LPSTR font, float x, float y, float scalex, float scaley, float * colour, int style) { ( ( void (*)( const char * text, int type, int font, float x, float y, float scalex, float scaley, float * colour, int style ) )d_DrawStringExt )( text, type, trap_R_RegisterFont((char*)font, 1), x, y, scalex, scaley, colour, style ); }
#pragma endregion

#pragma region Функции поиска адресов
...
#pragma region Инициализация переменных / поиск адресов
...
#pragma region Функции рисования объектов
void cdraw::Text(float x, float y, float size1, float size2, float *colour, int align, LPSTR font, const char *txtformat, ...)
{
	if (!init)
		initialization();
	...
	trap_R_DrawStringExt(szBuffer, 0x7FFFFFFF, font, x, y, size1, size2, colour, 3);
}
